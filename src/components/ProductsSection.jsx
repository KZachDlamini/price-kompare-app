...
function ProductsSection({ products, selectedCategory, onCategoryChange, onAddToList }) {
  // Define all possible categories including 'Staples'
  const categories = ["All", "Dairy", "Bakery", "Eggs", "Fruits & Veg", "Cleaning", "Staples"];

  return (
    <section className="products py-10 bg-gray-50">
      <div className="container mx-auto px-4">
        <h2 className="text-center text-3xl font-semibold text-gray-800 mb-8">Popular Deals</h2>
        ...
        <div className="hidden md:flex gap-3 flex-wrap justify-center mb-6">
          {categories.map(category => (
            <button
              key={category}
              className={`py-2 px-4 border border-gray-300 rounded-md font-medium transition duration-200
        ...
        <div className="md:hidden mb-6">
          <label htmlFor="categoryFilter" className="block text-lg font-medium text-gray-700 mb-2">Filter by Category:</label>
          <select
            id="categoryFilter"
        ...
        <div className="product-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.length === 0 ? (
            <p className="col-span-full text-center text-gray-600 text-lg">No products found for your selection.</p>
          ) : (
